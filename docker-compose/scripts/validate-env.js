...
// ── REPO AGENT LAUNCHER ───────────────────────────────────────────
// App service trong template này là Repo Agent Launcher UI.
// Khi ENABLE_REPO_AGENT=true (mặc định) → bắt buộc có Firebase RTDB
// connection để app khởi động được.
if ((env.ENABLE_REPO_AGENT || "true") === "true") {
  checkRequired(
    "REPO_AGENT_FIREBASE_DATABASE_URL",
    "Firebase RTDB URL (https://<project>-default-rtdb.<region>.firebasedatabase.app)",
    (v) => {
      try {
        const u = new URL(v);
        if (u.protocol !== "https:") return "must start with https://";
        if (!u.hostname.endsWith("firebasedatabase.app")) {
          return "expected hostname like *.firebasedatabase.app";
        }
        if (u.search || u.hash) {
          return "must be a plain URL without query/hash (no /env.json or ?auth=)";
        }
        return null;
      } catch {
        return "must be a valid https URL";
  ...
  checkRequired(
    ...
    (v) => {
      ...
      if (!cleaned || cleaned.length < 200 || !/^[A-Za-z0-9+/]+={0,2}$/.test(cleaned)) {
        return "must be a base64 string (≥200 chars)";
      }
      try {
        const json = Buffer.from(cleaned, "base64").toString("utf8");
        const obj = JSON.parse(json);
        if (!obj.client_email || !obj.private_key) {
          return "decoded JSON must contain client_email and private_key";
        }
        return null;
      } catch {
        return "decoded value is not valid JSON";
      }
    },
  );
  checkOptional("REPO_AGENT_MANAGER_HOST", "public host where the Repo Agent Manager UI is exposed");
  checkPort("REPO_AGENT_MANAGER_PORT", false);
  checkOptional("REPO_AGENT_TOTAL_SLOTS", "number of ttyd slots in the pool", (v) => {
    const n = Number(v);
